...
function PendingRTI({ setTab }) {
	const [data, setData] = useState([]);
	const { adminCollege, setRtiData } = useContext(SessionContext);
	const [loading, setLoading] = useState(false);
	async function handleRTI(e) {
		setLoading(true);
		var id = e.target.id;
		await axios
	...
	useEffect(() => {
		(async () => {
			setLoading(true);
			await axios
				.get(`http://localhost:5000/api/admin/pending/${adminCollege}`)
				.then((response) => {
					console.log(response.data);
